...
module tb_fc_dump;
    ...
    fc1_fc2_2dsp #(
        ...
        .fc1_valid_out(fc1_valid),
        .fc1_dout_dbg(fc1_dout_dbg),
    ...
    reg [8*STR_MAX-1:0] fc1_file;
    reg [8*STR_MAX-1:0] fc2_file;
    ...
    // ============================================================
    // TASK WRITE FC1 16 VALUE CSV
    // ============================================================
    task write_fc1_csv;
        input integer fd;
        input [FC1_OUT_SIZE*FC_OUT_WIDTH-1:0] vec;

        integer ch;
        begin
            for (ch = 0; ch < FC1_OUT_SIZE; ch = ch + 1) begin
                if (ch == FC1_OUT_SIZE-1)
                    $fwrite(fd, "%0d\n", vec[ch*32 +: 32]);
                else
    ...
    // ============================================================
    // TASK WRITE FC2 10 VALUE CSV
    // ============================================================
    task write_fc2_csv;
        input integer fd;
        input [FC2_OUT_SIZE*FC_OUT_WIDTH-1:0] vec;

        integer ch;
        begin
            for (ch = 0; ch < FC2_OUT_SIZE; ch = ch + 1) begin
                if (ch == FC2_OUT_SIZE-1)
                    $fwrite(fd, "%0d\n", vec[ch*32 +: 32]);
                else
                    $fwrite(fd, "%0d,", vec[ch*32 +: 32]);
            end
    ...
    // ============================================================
    initial begin
        if (!$value$plusargs("FC1_OUT=%s", fc1_file)) begin
            fc1_file = "fc1_out.csv";
        end

        if (!$value$plusargs("FC2_OUT=%s", fc2_file)) begin
            fc2_file = "fc2_out.csv";
        end

        ffc1 = $fopen(fc1_file, "w");
        ffc2 = $fopen(fc2_file, "w");

        if (ffc1 == 0) begin
            $display("ERROR: cannot open fc1 output file: %0s", fc1_file);
            $finish;
        end

        if (ffc2 == 0) begin
            $display("ERROR: cannot open fc2 output file: %0s", fc2_file);
            $finish;
        end

        $fwrite(ffc1, "fc1_0,fc1_1,fc1_2,fc1_3,fc1_4,fc1_5,fc1_6,fc1_7,fc1_8,fc1_9,fc1_10,fc1_11,fc1_12,fc1_13,fc1_14,fc1_15\n");
        $fwrite(ffc2, "c0,c1,c2,c3,c4,c5,c6,c7,c8,c9\n");
    end
    ...
    initial begin
        #10000000;
        $display("TIMEOUT FC DUMP");
        $display("fc1_count = %0d", fc1_count);
        $display("fc2_count = %0d", fc2_count);
        $fclose(ffc1);
        $fclose(ffc2);
        $finish;
    end
    ...
    always @(posedge clk) begin
        if (rst) begin
            ...
            if (fc1_valid && fc1_count < FC1_TOTAL) begin
                write_fc1_csv(ffc1, fc1_dout_dbg);
                fc1_count <= fc1_count + 1;
            end

            if (fc_valid && fc2_count < FC2_TOTAL) begin
                write_fc2_csv(ffc2, fc_dout);
                fc2_count <= fc2_count + 1;
            end

            if (fc1_count >= FC1_TOTAL && fc2_count >= FC2_TOTAL) begin
                $display("DONE FC DUMP");
                $display("fc1_count = %0d", fc1_count);
                $display("fc2_count = %0d", fc2_count);
