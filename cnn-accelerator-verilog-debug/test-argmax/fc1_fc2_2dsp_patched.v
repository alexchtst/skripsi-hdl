...
module fc1_fc2_2dsp #(
    ...
    // Debug output FC1: 16 hidden activation values, zero-extended to OUT_WIDTH
    output reg fc1_valid_out,
    output reg [FC1_OUT*OUT_WIDTH-1:0] fc1_dout_dbg,
    ...
    wire [ACT_SIZE-1:0] fc1_relu_value;
    wire [OUT_WIDTH-1:0] fc1_relu_ext;

    assign fc1_relu_value = relu_sat_act(fc1_sum_shifted);
    assign fc1_relu_ext   = {{(OUT_WIDTH-ACT_SIZE){1'b0}}, fc1_relu_value};
    ...
    always @(posedge clk) begin
        if (rst) begin
            ...
            // default: FC1 debug valid hanya pulse 1 cycle
            fc1_valid_out <= 1'b0;
                ...
                S_WRITE: begin
                    ...
                    if (!phase_fc2_reg) begin
                        if (last_pair_reg) begin
                            hidden[target_idx_reg] <= fc1_relu_value;

                            // Simpan FC1 activation ke packed debug vector sebagai 32-bit unsigned
                            fc1_dout_dbg[target_idx_reg*OUT_WIDTH +: OUT_WIDTH] <= fc1_relu_ext;
